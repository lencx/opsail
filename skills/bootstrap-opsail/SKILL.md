...
4. Select adapters:
   ...
   - Runtime Skill: use `npx --yes skills@latest` when Node.js is available and it names the current host as a supported agent (it does for OpenClaw, the default Hermes profile, Claude Code, and Codex). Otherwise use the host's native skill-install command. Without one, stage the Skill directory for a copy into the host's documented global skill directory.
...
If the runtime Skill mismatches, run only the command for the current host. With Node.js, set `OPSAIL_HOST_AGENT` to `openclaw`, `hermes-agent` (default profile only), `claude-code`, or `codex`, or to another agent name that `skills@latest` supports:

```sh
npx --yes skills@latest add https://github.com/lencx/opsail/tree/main/skills/opsail --global --agent "$OPSAIL_HOST_AGENT" --copy --yes
```

For a named Hermes profile or without Node.js, use the native command (prefix it with `hermes -p NAME` when needed):

```sh
hermes skills install https://raw.githubusercontent.com/lencx/opsail/refs/heads/main/skills/opsail/SKILL.md
```

Without Node.js, OpenClaw requires a local directory. Download and inspect the same runtime Skill into a temporary directory as `SKILL.md`, then run:

```sh
openclaw skills install "$TEMPORARY_OPSAIL_SKILL_DIRECTORY" --as opsail --global
```

Claude Code and Codex have no native install command. Without Node.js, stage and inspect the same `SKILL.md` in a temporary directory, set `OPSAIL_HOST_SKILLS_DIR` to `$HOME/.claude/skills` for Claude Code or `$HOME/.codex/skills` for Codex, then run:
